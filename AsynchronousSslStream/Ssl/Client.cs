...
namespace Trader.Server.Ssl
{
    public class Client
    {
        ...
        public Client(SslStream stream, Session id,byte[] buffer,Socket socket)
        {
            _Stream = stream;
            _Id = id;
            _Buffer = buffer;
            _Socket = socket;
            _WriteBufferIndex = BufferManager.TwoPartLength;
            _PartialReadIndex = BufferManager.ThreePartLength;
            Read();
        }
        ...
        {
            Task task = new Task(EndReadTaskAction,ar);
        ...
        private void Close()
        {
            ...
            this._Stream.Close();
            this._IsClosed = true;
            Application.Default.AgentController.EnqueueDisconnectSession(this._Id);
